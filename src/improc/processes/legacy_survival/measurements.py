...
def _load_nuclear_rois(workdir, well):
    ...
    # In the event no unknown region exists, use the nuclear region directly. In effect, this collapses the
    # unknown region into the nuclear region.
    try: unknown_rois = {ID : roi['unknown'] for ID, roi in rois.items()}
    except: unknown_rois = {ID : roi['nuclear'] for ID, roi in rois.items()}
...
def measure_rois(experiment: Experiment, to_be_measured: dict[str, set]):
    '''measure is a dict (map) from channels to list of measurements'''
    exp_name = experiment.label

    total_timepoints = len(experiment.mfspec.imaging_times)

    workdir = str(experiment.experiment_dir)
    ...
    for roi_path in roi_paths:
        ...
        stacked = experiment.datasets["stacked"]
        for channel in to_be_measured:
            stack = [image.data for image in stacked.images if image.vertex == well and well in image.path.name] # type: ignore

            if len(stack) == 0:
                print("couldn't find corresponding stack. This is probably a bad thing.")
                continue
            else:
                stack = stack[0]
            ...
            for ID, roi in rois.items():
                ...
                for ix, contour in enumerate(contours):
                    ...
                    # track of this. 
                        ...
                        # In the event that either dimension of the ROI is greater than the present dimensions,
                        # adjust ROI dimensions accordingly to be within image.
                        if y + h > stack[ix].shape[0]: y = stack[ix].shape[0] - h
                        if x + w > stack[ix].shape[1]: x = stack[ix].shape[1] - w
                ...
                if any(n_to_be_measured.values()) or any(c_to_be_measured.values()):
                    ...
                    try:
                        nuclear_roi = nuclear_rois[ID] # type: ignore
                    except KeyError: 
                        nuclear_roi = None
                    ...
                    if nuclear_roi is not None:
                        ...
                        for ix in range(len(nuclear_roi)):
                            ...
                            x, y, w, h = n_bound_rects[ID][ix] # type: ignore
                            n_subimgs.append(stack[ix][y:y+h, x:x+w])

                        if any(n_to_be_measured.values()):
                            measured.update(_take_measurements(n_to_be_measured[channel], n_subimgs, n_submasks[ID], nuclear_roi)) # type: ignore

                        if any(c_to_be_measured.values()):
                            measured.update(_take_measurements(c_to_be_measured[channel], subimgs, c_submasks[ID], None)) # type: ignore
...
class Measurement:
    ...
    gradient_std: bool = False
    aspect_ratio: bool = False

    def asitem(self) -> Tuple[str, set[str]]:
        args = set()
        if self.mean: args.add("mean")
        if self.std: args.add("std")
        if self.median: args.add("median")
        if self.pct_95: args.add("95th_percentile")
        if self.area: args.add("area")
        if self.perimeter: args.add("perimeter")
        if self.centroid: args.add("centroid")
        if self.skew: args.add("skew")
        if self.kurtosis: args.add("kurtosis")
        if self.gradient_mean: args.add("gradient_mean")
        if self.gradient_std: args.add("gradient_std")
        if self.aspect_ratio: args.add("aspect_ratio")
        return (self.channel.value, args)

class Measurements(Task):
