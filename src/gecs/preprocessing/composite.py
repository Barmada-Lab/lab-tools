...
from improc.experiment.types import Image, Axis, MemoryImage, Timepoint, Vertex, Channel, Exposure, Mosaic, Experiment
from improc.processes.types import OneToOneTask, Pipeline
...
def composite_survival(experiment: Experiment, ignore_channels: list[Channel] = []):
    ...
    for (vertex, mosaic), images in timeseries.items():
        ...
        for chan, images in chans.items():
            ...
            frames_reg = sr.transform_stack(frames, tmats=tmats)
